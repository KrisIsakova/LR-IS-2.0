...
class DiafilmApp(QMainWindow):
    ...
    def validate_inputs(self, data):
        ...
        for field in data:
            if not field:
                QMessageBox.warning(self, "Ошибка", "Все поля должны быть заполнены.", QMessageBox.Ok)
                return False

        # Проверка, что год является числом
        if data[4]:  # Проверка, что поле не пустое
            try:
                int(data[4])  # Год на титульнике
            except ValueError:
                QMessageBox.warning(self, "Ошибка", "Год на титульнике должен быть числом.", QMessageBox.Ok)
                return False

        if data[5]:  # Проверка, что поле не пустое
            try:
                int(data[5])  # Год на слайде
            except ValueError:
                QMessageBox.warning(self, "Ошибка", "Год на слайде должен быть числом.", QMessageBox.Ok)
                return False

        # Пример проверки строковых значений (если ожидается текст)
        if not isinstance(data[11], str):
            QMessageBox.warning(self, "Ошибка", "Наименование должно быть текстом.", QMessageBox.Ok)
            return False
    ...
    def save_new_data(self, ui_dob, dialog):
        try:
            ...
            if not self.validate_inputs(new_data):
                return
